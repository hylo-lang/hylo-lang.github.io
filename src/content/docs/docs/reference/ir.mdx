...
An instruction is a basic unit of computation in Hylo IR that modifies the state of the executing machine. An instruction may accept zero or more arguments and return zero or one value. An instruction has an associated type corresponding to instruction's result.
...
##### Semantics
When the `return` instruction is executed, the control flow returns back to the calling's function context.

##### See also
This is similar to [LLVM's `ret` instruction](https://llvm.org/docs/LangRef.html#ret-instruction), but it does not deal with the returned value. In Hylo IR, handling the return value happens prior to `return` instruction.
...
##### Syntax
```
<result> = alloca <storage-type>, <alignment>
<result> = alloca <storage-type>, #align(of: <t>)
```
...
`alignment` is an integer specifying the alignment in bytes.
...
- The alignment `alignment` or `#align(of: <t>)` must be compatible with the minimal alignment requirements for type `storage-type`.
...
##### See also
This is similar to [LLVM's `alloca` instruction](https://llvm.org/docs/LangRef.html#alloca-instruction), but it does not provide all the options.
...
- `source` must be initialized; after this instruction, `source` becomes uninitialized.
...
##### See also
This is similar to [LLVM's `load` instruction](https://llvm.org/docs/LangRef.html#load-instruction), but it does not provide all the options.
...
- `target` must be uninitialized; after this instruction, `target` becomes initialized.
...
##### See also
This is similar to [LLVM's `store` instruction](https://llvm.org/docs/LangRef.html#store-instruction), but it does not provide all the options.
...
##### Syntax
```
<result> = subfield <base> at <path> resulting <type>
```

Examples:
```
%i11: &ptr = subfield %i3 at [0, 0] resulting ptr
%i12: &Int = subfield %i3 at [0, 1] resulting Int
%i13: &ptr = subfield %b0#1 at [1] resulting ptr
```
...
##### See also
This is similar to [LLVM's `getelementptr` instruction](https://llvm.org/docs/LangRef.html#getelementptr-instruction), but it does not provide all the options.
