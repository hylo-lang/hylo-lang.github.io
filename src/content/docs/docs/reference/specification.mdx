...
2. Alignments are represented as values of the type `Int`. Valid alignments include only those values of `MemoryLayout<A>.alignment` for any type `A` plus the value of `Pointer.universal_alignment`. Every alignment value shall be a non-negative integral power of two.
...
1. Lambda type expressions have the form:
    ...
    ```ebnf
    lambda-type-expr ::=
      lambda-environment? '(' lambda-parameter-list? ')' receiver-effect? '->' type-expr
